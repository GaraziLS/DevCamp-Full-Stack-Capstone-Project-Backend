...
First, we start as usual, passing the route and the endpoint. Then, we request the user's data. Later, we perform a query on the user table and it checks if the username exists in the table, and pulls in the first result it finds. Then, a conditional checks yf the username and password are correct. If so, the user gets rendered (logs in), if not, a warning message appears.
...
Then, we'll override the title and content and id values by passing them to the variable that stores the request, we then will store that in variables and will create a new database session to commit the changes. Lastly, we'll parse everything with json:
...
> Note: When writing the id in Postman the system is taking the id of the existing items. If we get all the items
...
```
{
    ...
    {
        "item_content": "Edukiak",
        "item_title": "Izenburua",
        "item_user_id": 7
    }
```

> We'll see that there are only 4 records. If we try to query the seventh id, which is present in the fourth record, there will be issues.
...
> When deleting an item, the ids don't reorganize. If you delete the first 3 records from a list, despite having 4 remaining, they won't go from 1 to 4, but from 4 to 7.
...
Now we only have to deploy our endpoints to make them go live. We'll use [RAW](https://app.raw-labs.com/) to do that.